...
#Where to go
for (i in 1:19){ #from
  from = subset(D12,q101h1 == x[i])
  from = subset(from, q101j1 == 1)
  from = subset(from, select = c(procode, q101h1))
  from = na.omit(from)
  for (j in 1:31){ #to
    if(i == j){
    }
      ...
      areaY = Area$area[j];
      tmp = NULL
      tmp = data.frame(fromCode,toCode,rate,dis,areaX,areaY)
      tmp = merge(tmp, Data13,by.x = ("toCode"),by.y = ("Code"))
      combData = rbind(combData,tmp)
    }
  }
}

write.csv(combData,"2014InterCombData.csv")